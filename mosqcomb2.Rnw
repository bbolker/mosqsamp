...
$V$ is (unordered) occupancy of a particular configuration
...
We have $\sum s_i=B$, $\sum i s_i = M$.

Define the multinomial coefficient $\multinom(S) \equiv 
\binom{(\sum s_i)!}{\prod s_i!}$.
...
<<sim1>>=
rfun0 <- function(B,M) table(factor(sample(1:B,size=M,replace=TRUE),levels=1:B))
@ 
...
benchmark(rfun(1000,50),rfun0(1000,50),
